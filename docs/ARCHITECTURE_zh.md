...
**譯文在夜讀下讀得出來。** 素材是翻譯引擎的真實輸出，對它跑完整夜讀管線：

| 量測 | 值 |
|---|---|
| 字筆畫成品亮度 | 238.7（上限 240） |
| 泡底成品亮度 | 16.1 |
| 對比 | +222.6 |
| 六顆主要泡對比不足的 | 0 顆 |

譯文比原生手寫字更好處理：純色黑字墨度高、邊緣乾淨，低墨度的壓黑點一刀切下去剛好。

量測本身有個陷阱。直接量「筆畫遮罩 ∩ 泡」會得到 96.5、看起來偏暗，因為那個遮罩裡有 60.4%
...
指標上 C 最好，但**看圖否決**，兩種失效模式：

1. **泡填不滿。** DBNet 的 seg 是文字**區域**，某顆泡該處覆蓋 90%，而精確筆畫只有 32% 是黑。
   氣泡的核心填色拿 seg 當種子，種子小了泡就填不滿，右下角留一塊灰。
2. **整顆泡漏掉。** 裝飾性的手寫字完全不在任何文字區內。排版器只知道自己畫了什麼，看不到
   沒被翻譯的字，而不翻譯裝飾性 SFX 是 manga-image-translator 的既定政策。
